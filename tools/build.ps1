...
try {
    ...
    & (Join-Path (Split-Path $cmake) 'ctest.exe') --test-dir build -C $Configuration -R '^(camera_mailbox|scene_ir_dson|morph_runtime|morph_catalog|morph_compatibility|project_settings|skeleton_pose|formula_runtime|scene_workflow|conform_runtime|attachment_workflow|lazy_morph|render_options|parameter_controls|content_browser|subdivision)$' --output-on-failure
