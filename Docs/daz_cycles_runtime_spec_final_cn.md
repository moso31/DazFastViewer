...
## 本地开发环境与版本基线（用户补充，2026-09-23 更新）

- **007 后视口与角色交互优化**：尺寸／停靠变化复用 Cycles 会话和场景，角色移动接入交互预览，碰撞按实际相对变化求值，选取／高亮按网格与实例增量更新。按用户要求使用 `test3.duf` 在副屏对比：6 像素缩放约 99 毫秒出现新图、262 毫秒恢复完整分辨率；角色平移／旋转约 96–101 毫秒出现新位置、停止后约 186–206 毫秒恢复完整分辨率。原版两轮分别存在约 7.1–7.8 秒尺寸等待及约 19–21 秒角色恢复等待。原始证据、回归和口径见[优化执行记录](interaction_optimization_execution_cn.md)。降噪关闭、最终采样不变，以上不是噪点收敛时间，008 未开始。

- **内容浏览器紧凑布局与交互等待诊断**：图标改为 64–224 像素、每 8 像素一档，另保留列表，迁移原有配置；顶部控件合并为一行，去掉统计 / 路径说明行和视口 guide。最近 `6.duf` 会话发现尺寸变化伴随 45.8 / 52.1 秒场景同步，角色移动另有全场景选取 / 高亮重建等可优化路径；见[交互分析](interaction_latency_analysis_cn.md)。本轮未修改渲染算法，008 未开始。

- **007 后内容浏览器增强**：增加默认中等图标、Ctrl＋滚轮切换及布局配置保存，后续细化为 21 档图标及列表；支持同名 PNG / TIP 悬停预览、跨内容库的路径包含搜索及最近 10 个搜索词、按 DUF 类型分类的近期使用（每类 20 个、所有 100 个）。H:/G1 与 H:/G3 共 175,583 个 DUF，索引就绪后的查询响应 31–63 毫秒；首次建库约 38.8 秒，重启索引读取约 313 毫秒。实现、刷新边界及验证见[内容浏览器报告](content_browser_execution_cn.md)，008 未开始。剩余交互性能问题已整理为[下一轮需求](interaction_optimization_request_cn.md)。

- **007 后视口导航预览修复**：编辑器在鼠标导航和 WASDQE 持续操作期间使用宽、高各 1/4、最多 2 样本的预览；滚轮 / 聚焦先呈现预览，静止后恢复完整分辨率和正式采样。后续按用户体验反馈恢复 Shift＋右键平移，Ctrl＋右键转头；增加中键平移、后侧键拖动转头及单击聚焦，与 Hierarchy 选择联动。详见[预览报告](navigation_preview_execution_cn.md)和[单手操作报告](one_hand_navigation_execution_cn.md)，008 未开始。

- **原始采样收敛更正**：按用户要求关闭视口与离线降噪，采用蓝噪声采样；视口上限 4096，自适应阈值保留 0.01。真实 test.duf 的 15 秒原始画面较旧序列改善，但尚未达到用户描述的 Iray 无降噪标准；历史 3.43 秒降噪结果不能作为收敛证明。13 项工程检查及离线输出通过，详情见[无降噪复测](raw_convergence_execution_cn.md)。

- **test.duf 本轮复测修复**：补齐 HDRI 背景光重要性采样并统一离线 PNG 色调；修正 Fit To 重复位移、Rigid Follow 小附件与对象局部轴心。滑块可越过标尺范围，右侧输入不再被后台刷新覆盖。13 项工程回归通过，真实原生 DUF 的初始挂接、移动 / 姿态跟随和恢复通过；拖鞋相对现有 DAZ 几何参考 RMS 从 11.318 降至 0.779 毫米，仍保留局部及跨渲染器差异。详情见[本轮报告](test_scene_repair_execution_cn.md)，采样设置以上述更正为准。008 未开始。
...
- **test.duf 可见性、发丝、材质及上衣贴合修复**：接入保存的 Visible 与场景树 / 属性开关，支持原生 StrandBasedHair 曲线及蒙皮，修复根层级 FitTo 拖鞋，补充皮肤 / 头发 / 薄壁眼部材质。A 上衣增加蒙皮后的碰撞和 GeoGraft 表面约束。验证、近似边界与最终证据见[本轮报告](scene_fidelity_execution_cn.md)，不计为 008。

- **用户视觉复测后的形变修复**：修正保存值提前限幅、骨骼缩放补偿与 Figure 缩放基值，保留带 HD / 缺失附件输出的基础形态，隔离嵌套 Figure 姿势并平滑生成的服装位移。两个参考角色头部与 DAZ DBZ 误差约 0.0002 毫米；服装仍有局部近似差异，不能视作完整 Golden。见[形变修复报告](scene_deformation_execution_cn.md)。此前加载 / 渲染 PASS 不等同于视觉验收。

- **007 后追加场景删除与 DUF 兼容性修复**：增加模型 / 灯光删除与空场景入口，统一回收无引用资源并恢复渲染失败后的场景切换；DUF 按复合内容分派，内嵌派生几何沿拓扑兼容的 `source` 继承 Morph / 骨架。真实 `H:\G1\Scenes\3.duf` 的 Local 刚性绑定及冗余缩放图兼容也已补齐，验证见[本轮执行报告](scene_lifecycle_execution_cn.md)。008 未开始。
