...
2026-09-25：T 新增 XY／XZ／YZ 平面手柄；T、R 分别记忆本次运行中的 Local／World 状态。T／R／S 未命中手柄时保留左键 IK，命中手柄仍优先执行 Gizmo。

Local／World 右侧新增独立“角色功能模块”，提供小人向下落地图标和每个角色独立的地面对齐比例（默认 0）。“编辑 → 对齐到地面”及 Ctrl+D 使用当前世界 AABB，沿 DAZ 世界 Y 移动到 `minY = 比例 × (maxY - minY)`；等待最新求值、支持旋转／缩放父级，重复执行不累积偏移。比例编辑本身不重算几何；当前场景内切换、追加、刷新及删除其他角色时保留，重新打开场景默认 0。
...
2026-09-25：已实现单选节点的平移、Local／World 旋转、单轴／等比缩放，覆盖现有可编辑模型、骨骼和灯光。拖动使用灰模代理，松手提交，Esc／失焦／选择或版本变化取消；保留骨骼锁定和限位，并修复 ±90° 欧拉奇点的世界旋转。
...
2026-09-25：按用户要求移除“预设应用详情”和“可见（Visible）”控件，保留“重置选中对象”；同步删除失效的“查看详情”指引。场景树显隐继续可用，20 项工程检查与副屏真实角色隐藏／恢复通过。新版已部署，SHA-256 为 `d27b36da31a636f803253e3276ab53241f631169f32105aeedb4d54fc15322a2`，161 项源码哈希一致。详见 [控件精简记录](Docs/property_controls_cleanup_cn.md)。已有 PowerPose 紧凑布局改动保留，本轮未提交 Git。
